...
my $ref16RNAdb_1 = $bin_dir . "/databases/silva_release_138_1-bac-16s-id90.fasta";
my $ref16RNAdb_2 = $bin_dir . "/databases/silva_release_138_1-arc-16s-id95.fasta";
my $ref18db = $bin_dir . "/databases/silva_release_138_1-euk-18s-id95.fasta";
...
if ($reference_sequence ne "16S" && $reference_sequence ne "18S") {
    ...
    unless (-e $arb_file) {  # if arb file does not exist
        ...
        system $bin_dir . "/sina-1.7.2-linux/sina -i $reference_sequence --prealigned -o $arb_file 2>&1";
    }
} else {
    # use default silva reference arb file
    $arb_file = $silva_db_arb;
}
system $bin_dir . "/sina-1.7.2-linux/sina --in $pathout/$nonchimeric_otus_03_filtered_filename --out $pathout/$nonchimeric_otus_03_aligned_filename --db $arb_file --search --intype=fasta --outtype=fasta --fasta-write-dna --lca-fields=tax_slv, --meta-fmt csv >/dev/null 2>&1";
