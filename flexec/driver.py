...
class Driver(object):
    ...
    @abstractmethod
    def run_cmd(self, cmd: List[str]) -> Iterator[bytes]:
        pass
...
class LinuxDriver(ClassicDriver):
    def __init__(self):
        super().__init__()
        logging.debug(f'using driver LinuxShellDriver')


class WindowsCmdDriver(ClassicDriver):
    def __init__(self):
        super().__init__()
...
class WSLDriver(Driver):
    def __init__(self, distro_name='ngstoolkitdist', wsl_distro_version=2):
        ...
        self.cmd_prefix = ['wsl', '-d', self.distro_name, '--']
        self.shellDriver = WindowsCmdDriver()
        self.shellDriver.self_check()
